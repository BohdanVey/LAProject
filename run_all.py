...
if __name__ == '__main__':
    ...
    photos_dir = 'correct/'
    result_dir = 'results/'
    total_file = f'out_all{SVD_rank}.txt'
    filename = f'out{SVD_rank}.txt'
    k = 0
    ...
    with open(os.path.join(result_dir, total_file), 'w') as tf:
        for i in os.listdir(photos_dir):
            ...
            for q in range(3):
                values, left_s, rigth_s = svd(img_correct[:, :, q].astype('float64'), SVD_rank)
            ...
            st_rsvd = time.time()
            for q in range(3):
                values, left_s, rigth_s = rSVD(img_correct[:, :, q].astype('float64'), SVD_rank)
            ...
            tf.write(f"{i}\n")
            tf.write(f"SVD square loss: {svd_loss[0]}\n")
            tf.write(f"SVD dice loss: {svd_loss[1]}\n")
            tf.write(f"SVD time(sec): {svd_time}\n")

            tf.write(f"random SVD square loss: {rsvd_loss[0]}\n")
            tf.write(f"random SVD dice loss: {rsvd_loss[1]}\n")
            tf.write(f"random SVD time(sec): {rsvd_time}\n")

            tf.write(f"Neural network square loss: {neural_loss[0]}\n")
            tf.write(f"Neural network dice loss: {neural_loss[1]}\n")
            tf.write(f"Neural time(sec): {neural_time}\n")

            with open(os.path.join(result_dir, filename), 'w') as f:
                f.write(f"SVD square loss: {total_svd_square_loss / k}\n")
                f.write(f"SVD dice loss: {total_svd_dice_loss / k}\n")
                f.write(f"SVD total time(sec): {total_svd_time}\n")

                f.write(f"random SVD square loss: {total_rsvd_square_loss / k}\n")
                f.write(f"random SVD dice loss: {total_rsvd_dice_loss / k}\n")
                f.write(f"random SVD total time(sec): {total_rsvd_time}\n")

                f.write(f"Neural network square loss: {total_neural_square_loss / k}\n")
                f.write(f"Neural network dice loss: {total_neural_dice_loss / k}\n")
                f.write(f"Neural network total time(sec): {total_neural_time}\n")
